...
fn fractal(_p: vec2<f32>) -> f32 {
    var p = abs( 5.0 - mod_2( _p*0.2, 10.0 ) ) - 5.0;
    var ot = 1000.;
    for (var i = 0; i < 7; i+=1) {
...
fn march(_from: vec3<f32>, dir: vec3<f32>, frag_coord: vec2<f32>) -> vec3<f32> {
    var p = vec3<f32>(0.);
    var n = vec3<f32>(0.);
    var g = vec3<f32>(0.);
    var d = 0.0;
...
fn lookat(_dir: vec3<f32>, up: vec3<f32>) -> mat3x3<f32> {
    let dir = normalize(_dir);
...
fn shader_main(frag_coord: vec2<f32>) -> vec4<f32> {
    ...
    dir=lookat(adv-_from, vec3<f32>(0.0, 1.0, 0.0)) * dir;
    let col=march(_from, dir, frag_coord);
